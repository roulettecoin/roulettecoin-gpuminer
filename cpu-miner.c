...
static void *miner_thread(void *userdata)
{
	...
	struct work work = {{0}};
	uint32_t max_nonce;
	uint32_t end_nonce = ((((1ULL << (32 - opt_intensity)) / opt_n_threads) * (thr_id + 1)) << opt_intensity) - 1;
	char s[16];
	int i;
	...
	while (1) {
		...
		if (memcmp(work.data, g_work.data, 76)) {
			...
			work.data[19] = (((1ULL << (32 - opt_intensity)) / opt_n_threads) * thr_id) << opt_intensity;
		}
		pthread_mutex_unlock(&g_work_lock);
		...
		if (rc) {
			...
			// continue from the correct nonce
			work.data[19] &= ~((1 << opt_intensity) - 1);
			work.data[19] += 1 << opt_intensity;
		}
	}
